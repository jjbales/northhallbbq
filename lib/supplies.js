...
const recordPurchase = db.transaction((supplyId, qty, totalCents, note, packs) => {
  ...
  db.prepare('INSERT INTO supply_purchases (supply_id, qty, total_cents, note, packs) VALUES (?,?,?,?,?)')
    .run(supplyId, qty, Math.round(totalCents), note || '', packs > 0 ? packs : 0);
