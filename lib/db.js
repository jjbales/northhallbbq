...
-- Every meat run. Pieces and pounds both, because you buy by the pound and
-- cook by the piece, and you want to see either one.
CREATE TABLE IF NOT EXISTS protein_purchases (
  id           INTEGER PRIMARY KEY AUTOINCREMENT,
  protein_id   INTEGER NOT NULL REFERENCES proteins(id) ON DELETE CASCADE,
  qty          REAL NOT NULL,                  -- how many pieces
  lbs          REAL NOT NULL DEFAULT 0,        -- total weight, optional
  total_cents  INTEGER NOT NULL,
  note         TEXT DEFAULT '',
  purchased_at TEXT NOT NULL DEFAULT (datetime('now'))
);

-- The album. Web-sized copies live in public/photos; the originals stay put.
CREATE TABLE IF NOT EXISTS photos (
  id         INTEGER PRIMARY KEY AUTOINCREMENT,
  file       TEXT NOT NULL UNIQUE,           -- "/photos/brisket1.jpg"
  thumb      TEXT NOT NULL DEFAULT '',       -- "/photos/brisket1-thumb.jpg"
  caption    TEXT DEFAULT '',
  protein_id INTEGER REFERENCES proteins(id) ON DELETE SET NULL,
  featured   INTEGER NOT NULL DEFAULT 0,     -- show it on the shop page too
  active     INTEGER NOT NULL DEFAULT 1,
  sort_order INTEGER NOT NULL DEFAULT 0,
  added_at   TEXT NOT NULL DEFAULT (datetime('now'))
);

-- What customers say. Nothing reaches the public page until it's approved --
-- an open comment box on a business site is a spam magnet otherwise.
CREATE TABLE IF NOT EXISTS feedback (
  id           INTEGER PRIMARY KEY AUTOINCREMENT,
  name         TEXT NOT NULL,
  rating       INTEGER,                      -- 1-5, optional
  body         TEXT NOT NULL,
  email        TEXT DEFAULT '',              -- private, so you can reply
  phone        TEXT DEFAULT '',              -- private
  order_public_id TEXT DEFAULT '',           -- set when it comes off a confirmation page
  status       TEXT NOT NULL DEFAULT 'pending', -- pending | approved | hidden
  reply        TEXT DEFAULT '',              -- your public answer, optional
  ip_hash      TEXT DEFAULT '',              -- for rate limiting only
  created_at   TEXT NOT NULL DEFAULT (datetime('now')),
  reviewed_at  TEXT
);
...
// Cold storage. Meat gets bought in bulk and sits in the freezer, so each
// protein carries a count on hand and a running average of what it cost --
// same idea as the pantry, different shelf.
addColumn('proteins', 'on_hand', 'REAL NOT NULL DEFAULT 0');         // pieces in the freezer
addColumn('proteins', 'unit_cost_cents', 'REAL NOT NULL DEFAULT 0'); // weighted average per piece
addColumn('proteins', 'avg_lbs', 'REAL NOT NULL DEFAULT 0');         // average raw weight per piece

// Where a supply comes from, so the pantry can link you straight to the price.
// item_url is a pinned product page; without one the link falls back to a
// search on the supplier's site, or to its front door when the catalogue is
// behind a login.
addColumn('supplies', 'supplier', "TEXT NOT NULL DEFAULT ''");
addColumn('supplies', 'item_url', "TEXT NOT NULL DEFAULT ''");

// You buy rub by the container and use it by the ounce. pack_qty is how many
// stock units come in one of whatever you actually hand over money for; 0
// means you buy it in the same unit you use it in. pack_unit is what to call
// that thing on screen -- container, roll, case.
addColumn('supplies', 'pack_qty', 'REAL NOT NULL DEFAULT 0');
addColumn('supplies', 'pack_unit', "TEXT NOT NULL DEFAULT ''");
// how many packs a receipt was, so "6 containers" survives in the log rather
// than being flattened to the 156 oz it became
addColumn('supply_purchases', 'packs', 'REAL NOT NULL DEFAULT 0');
addColumn('proteins', 'low_at', 'REAL NOT NULL DEFAULT 0');
addColumn('cook_proteins', 'consumed', 'INTEGER NOT NULL DEFAULT 0');

// Artwork. Each protein ships with a drawn badge; drop your own photo on a
// product and it takes over for that item.
addColumn('proteins', 'art', "TEXT DEFAULT ''");
addColumn('products', 'image_url', "TEXT DEFAULT ''");

// Customer-submitted photos. They live on the persistent disk rather than in
// public/, so an unapproved one is never reachable by guessing a URL.
addColumn('photos', 'source', "TEXT NOT NULL DEFAULT 'owner'");   // owner | customer
addColumn('photos', 'feedback_id', 'INTEGER');
addColumn('photos', 'stored', "TEXT DEFAULT ''");                  // filename under DATA_DIR/uploads
addColumn('photos', 'credit', "TEXT DEFAULT ''");                  // "photo by Dale P."
addColumn('photos', 'width', 'INTEGER NOT NULL DEFAULT 0');
addColumn('photos', 'height', 'INTEGER NOT NULL DEFAULT 0');
addColumn('feedback', 'upload_token', "TEXT DEFAULT ''");
...
const DEFAULT_SETTINGS = {
  ...
  // What a group pays you per butt to cook it, on top of their meat and
  // supplies. It steps down once a cook gets big. Drives the estimator.
  fundraiser_fee_first: '1.00',
  fundraiser_fee_break_at: '250',
  fundraiser_fee_after: '0.50',
  // The album and the comment box.
  gallery_headline: 'Off the pit',
  gallery_blurb: 'Every one of these came off my smoker. No stock photos.',
  feedback_headline: 'How did we do?',
  feedback_blurb: "Tell me what you thought — good or bad, I read every one of them. I'll ask before putting your name on the site.",
  feedback_thanks: "Thank you. I read every one of these myself, and I'll post it once I've had a look.",
  show_feedback: '1',
};
...
function seed() {
  ...
  // point each protein at its drawn badge if it hasn't got one
  db.prepare("UPDATE proteins SET art = '/img/' || slug || '.svg' WHERE art IS NULL OR art = ''").run();
  ...
  // Pick up any photos sitting in public/photos and guess what each one is of
  // from its filename. Runs once, on an empty album -- after that it's yours to
  // manage, and nothing here overwrites a caption or a tag you've set.
  if (db.prepare('SELECT COUNT(*) c FROM photos').get().c === 0) {
    ...
    const pics = files.filter((f) => /\.(jpe?g|png|webp)$/i.test(f) && !/-thumb\./i.test(f)).sort();

    const guess = (name) => {
      const n = name.toLowerCase();
      if (/brisket|packer/.test(n)) return brisket.id;
      if (/chicken|bird|spatch/.test(n)) return chicken.id;
      if (/pork|butt|shoulder|pulled/.test(n)) return pork.id;
      return null;
    };
    const ins = db.prepare(
      `INSERT OR IGNORE INTO photos (file, thumb, caption, protein_id, featured, sort_order)
       VALUES (?,?,?,?,?,?)`
    );
    pics.forEach((f, i) => {
      ...
      ins.run(`/photos/${f}`, thumb, '', guess(base), 1, i + 1);
    });
  }
}
seed();
